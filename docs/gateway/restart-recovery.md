...
seconds. It refreshes cached health and presence immediately, then restarts
channel connections once tracked Gateway work is idle. This keeps stale sockets
from waiting for their normal expiry without interrupting an active reply or
agent startup when a busy event loop caused the timer gap.
