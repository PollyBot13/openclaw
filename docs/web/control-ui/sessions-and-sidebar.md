...
Background session-list updates are coalesced during activity, with at most one refresh per second for each query. **Refresh** and returning to a hidden page can refresh immediately.
